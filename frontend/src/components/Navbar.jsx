...
const Navbar = () => {
  ...
  return (
    <>
      {/* -------- Desktop Navbar -------- */}
      <div className='hidden sm:flex items-center justify-between py-4 px-10 shadow-sm bg-white font-medium z-50 relative'>
          ...
          <div className='relative group'>
            ...
            {token && (
              <div className='hidden group-hover:flex flex-col absolute right-0 top-8 w-52 bg-white border shadow-lg rounded-md z-10'>
                <p className='py-2 px-4 text-md hover:bg-gray-100 text-gray-600 text-left'>My Account</p>
                <button className='py-2 px-4 text-md hover:bg-gray-100 text-gray-600 text-left' onClick={() => navigate('/orders')}>Orders</button>
                <button className='py-2 px-4 text-md hover:bg-gray-100 text-red-500 text-left' onClick={logout}>Logout</button>
              </div>
            )}
          </div>
      ...
      {/* -------- Mobile Navbar -------- */}
      <div className='flex sm:hidden items-center justify-between py-4 px-5 shadow-sm bg-white font-medium z-50 relative'>
        ...
        <div className='flex flex-col text-gray-700'>
          ...
          {token && (
            <>
              ...
              <button onClick={() => { logout(); setVisible(false); }} className='py-3 px-6 border-b text-left text-red-500 hover:bg-gray-50'>Logout</button>
            </>
          )}
        </div>
      </div>
